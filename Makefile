...
OPTIONS=-D$(RADIOSITY_MODE) -D$(FORM_FACTOR_CALCULATION) -DHEMICUBE_GRID_SIZE=$(HEMICUBE_GRID_SIZE) -DHEMICUBE_NEAR_PLANE=$(HEMICUBE_NEAR_PLANE) -DDIFF_TO_TOTAL_CUTOFF=$(DIFF_TO_TOTAL_CUTOFF)

CC=g++
CFLAGS=-c -Wall -I$(INCLUDE_DIR) $(OPTIONS)
LDFLAGS=
SRC_DIR=src
BUILD_DIR=build
INCLUDE_DIR=include
